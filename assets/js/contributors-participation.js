...
async function load(){
 ...
 if(error){
  list.innerHTML='<p class="muted">Leaderboard unavailable right now.</p>';
  if(mineCard)mineCard.innerHTML=session?.user?'<p class="muted">Your position is unavailable right now. Please try again shortly.</p>':'<p class="muted">Sign in to see your position and share your status.</p><a class="btn primary" href="account.html">Join the experiment</a>';
  return;
 }
 const hasRows=Array.isArray(data)&&data.length>0;
 if(!hasRows)list.innerHTML='<div class="leaderboard-empty"><strong>The leaderboard is waiting.</strong><span>Be one of the first contributors.</span><a class="btn primary" href="account.html">Join the experiment</a></div>';
 ...
 if(hasRows)list.innerHTML=topRows.map((p,i)=>{
