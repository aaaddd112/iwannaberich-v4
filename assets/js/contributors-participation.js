...
async function load(){
 if(!sb||!list)return;
 const {data,error}=await sb.rpc('get_public_leaderboard',{p_limit:25});
 ...
 list.innerHTML=rows.map((p,i)=>{const visitors=Number(p.qualified_visitors||0);const score=Number(p.xp||0);const badge=Number(p.achievements||0);return `<article class="leaderboard-row"><div class="leaderboard-position">${String(i+1).padStart(2,'0')}</div><div class="leaderboard-identity"><strong>#${esc(p.contributor_number)} ${esc(p.display_name||p.username)}</strong><span>@${esc(p.username)} · ${esc(p.rank||rank(score))}</span></div><div class="leaderboard-growth"><strong>${fmt(visitors)}</strong><span>people brought</span></div><div class="leaderboard-score"><strong>${fmt(score)}</strong><span>score</span></div><div class="leaderboard-badges">${fmt(badge)} ${badge===1?'badge':'badges'}</div></article>`}).join('');
}
