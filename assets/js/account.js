(() => {
  ...
  function addRecoveryViews() {
    ...
    $("recoveryForm").addEventListener("submit", async e => {
      e.preventDefault(); const email=$("recoveryEmail").value.trim(); if(!email)return;
      const button=e.currentTarget.querySelector("button"); button.disabled=true; message("Sending recovery email…");
      try { const {error}=await sb.auth.resetPasswordForEmail(email,{redirectTo:`${location.origin}/account.html`}); if(error) return message("We couldn't send the recovery email. Please try again later.","error"); message("If an account exists for that email, a recovery link has been sent.","success"); e.currentTarget.reset(); }
