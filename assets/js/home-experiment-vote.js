...
(function () {
  ...
  function apiOptions(body) {
    return { method: 'POST', headers: { apikey: SUPABASE_KEY, Authorization: 'Bearer ' + SUPABASE_KEY, 'Content-Type': 'application/json' }, body: JSON.stringify(body || {}) };
  }
  ...
  function bind() {
    ...
    function render(rows) {
      ...
      var total = rows.reduce(function (sum, row) { return sum + Number(row.vote_count || 0); }, 0), counts = {};
      ...
      buttons.forEach(function (button) {
        var count = counts[button.dataset.option] || 0, pct = total ? Math.round(count / total * 100) : 0;
        var strong = button.querySelector('.experiment-option-result strong'), small = button.querySelector('.experiment-option-result small');
      ...
      buttons.forEach(function (button) { button.classList.toggle('is-selected', button.dataset.option === chosen); button.disabled = Boolean(chosen); });
    }

    function load() {
      return fetch(READ_ENDPOINT, apiOptions()).then(function (response) { if (!response.ok) throw new Error('Vote totals unavailable'); return response.json(); }).then(render).catch(function () { if (totalEl) totalEl.textContent = 'Live vote totals temporarily unavailable.'; });
    }
    ...
    buttons.forEach(function (button) {
      button.addEventListener('click', function () {
        ...
        fetch(VOTE_ENDPOINT, apiOptions({ option_code: button.dataset.option })).then(function (response) {
          if (!response.ok) throw new Error('Vote could not be recorded');
          try { localStorage.setItem(VOTE_KEY, button.dataset.option); } catch (e) {}
          if (window.IWBRAnalytics && window.IWBRAnalytics.trackEvent) window.IWBRAnalytics.trackEvent('homepage_experiment_vote', { option: button.dataset.option });
          return load();
